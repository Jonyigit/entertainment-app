...
import sliderCardStyle from "./sliderCard.module.scss";
import playIcon from "../../../assets/icon/play.svg";

function SliderCard({ title, category, img, rating, year, isBookmarked, id }: TProps) {
    return (
        <div
            className={sliderCardStyle.card}
            style={{
                backgroundImage: `url(${img})`,
            }}
        >
            <div className={sliderCardStyle.bg}>
                <div className={sliderCardStyle["play-box"]}>
            ...
            <div className={sliderCardStyle["movie-info"]}>
                <div className={sliderCardStyle["movie-content"]}>
                    <p className={sliderCardStyle.year}>{year}</p>
                    <ul>
                        <li>
                            {category === "Movie" ? (
                                <svg
                ...
                <h2 className={sliderCardStyle["cinema-name"]}>{title}</h2>
            </div>
        </div>
    );
}
